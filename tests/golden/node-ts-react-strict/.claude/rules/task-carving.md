...
paths:
  - "**/*"
---
# Task carving & context discipline

- **One carved task per session.** Each session targets a single, module-scoped
  task with a clear definition of done, recorded as the "Goal" in the current
  handoff (`.claude/handoffs/current.md`).
- **Start from the handoff, not a repo scan.** The handoff's "Context pointers"
  list is the only required reading. Open those files; do not re-scan the whole
  repository to rebuild context.
