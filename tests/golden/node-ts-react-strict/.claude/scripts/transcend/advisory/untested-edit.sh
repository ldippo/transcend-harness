#!/usr/bin/env sh
# transcend PostToolUse advisory hook: after editing a source (non-test) file, remind
...
SESSION_ID="$(transcend_json_file_get "$TMP" session_id)"
transcend_once "$SESSION_ID" "untested-edit" || exit 0

transcend_emit_context PostToolUse "Source files are changing but no test file has been touched yet this session. The testing convention (.claude/rules/testing.md) expects behavioral changes to ship with tests."
exit 0
