#!/usr/bin/env sh
# fable PreToolUse blocking hook (generic push gate): before a `git push`, run a
...
if [ $STATUS -ne 0 ]; then
  TAIL="$(printf '%s\n' "$OUT" | tail -15)"
  fable_emit_deny "Push blocked: $LABEL gate failed (\`$GATE_CMD\` exited $STATUS). Fix before pushing. Output tail:
$TAIL"
fi
exit 0
