#!/usr/bin/env sh
# transcend Stop advisory hook: remind (once per session) when work is happening
...
transcend_once "$SESSION_ID" "on-protected-branch" || exit 0

transcend_emit_context Stop "You are working directly on '$PROTECTED' with uncommitted changes. The git workflow (.claude/rules/git-workflow.md) expects a branch per change — consider moving this work to a feature branch."
exit 0
