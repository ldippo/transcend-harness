#!/usr/bin/env sh
# transcend manifest verifier. Read-only. Compares the on-disk .claude/ tree against
# .claude/.transcend/manifest.json and emits a JSON drift report on stdout:
...
manifest_path = os.path.join(claude_dir, ".transcend", "manifest.json")

if not os.path.isfile(manifest_path):
    print(json.dumps({"manifest_present": False,
                      "error": "no .claude/.transcend/manifest.json (hand-built harness?)"}))
    sys.exit(0)
